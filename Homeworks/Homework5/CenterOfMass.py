...
# and simulation snapshot
    ...
    def COMdefine(
        self,
        a:float|np.ndarray[np.float64]|u.Quantity,
        b:float|np.ndarray[np.float64]|u.Quantity,
        c:float|np.ndarray[np.float64]|u.Quantity,
        m:float|np.ndarray[np.float64]|u.Quantity
    ...
    def COM_P(self, delta:float) -> u.Quantity:
        '''
        ...
        r_COM = np.linalg.norm(np.vstack((x_COM,y_COM,z_COM)),axis=0)
        ...
        while (change > delta):
            ...
            # write your own code below
            r_COM2 = np.linalg.norm(np.vstack((x_COM2,y_COM2,z_COM2)),axis=0)
    ...
    def COM_V(self,x_COM:u.Quantity, y_COM:u.Quantity, z_COM:u.Quantity) -> u.Quantity:
        ...
        rV = np.linalg.norm(np.vstack((xV,yV,zV)),axis=0)
...
def main() -> None:
    ...
    delta_r_MW = np.linalg.norm(MW_COM_p-M31_COM_p)
    print(f"The separation between MW and M31 is \n{delta_r_MW:.3f}")

    delta_v_MW = np.linalg.norm(MW_COM_v-M31_COM_v)
    print(f"The relative velocity of MW with respect to M31 is \n{delta_v_MW:.3f}")

    print('\nQUESTION 3\n')
    
    delta_r_M33 = np.linalg.norm(M33_COM_p-M31_COM_p)
    print(f"The separation between M33 and M31 is \n{delta_r_M33:.3f}")

    delta_v_M33 = np.linalg.norm(M33_COM_v-M31_COM_v)
