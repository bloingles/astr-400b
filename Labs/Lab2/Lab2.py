...
def schechter_M(m:np.ndarray,phi_star:float=0.0166,m_star:float=-23.19,alpha:float=-0.81) -> float:
...
def schechter_L(lum:np.ndarray, n_star:float=8e-3, l_star:float=1.4e10, alpha:float=-0.7) -> float:
    """ Function that computes the Schechter Luminosity Function
...
def imf(m:np.ndarray, m_min:float=0.1, m_max:float=120, alpha:float=2.35) -> float:
...
def main() -> None:
    ...
    ax.semilogy(mk,schechter_M(mk,alpha=-0.6),color='k',
    ...
    print(f"Ratio (>L*)/LTotal = {np.round(ratio,3)}")

    ### Part B: IMF ###

    ### Q1 solutions
    test = quad(lambda m: imf(m),0.1,120) # Integrate the imf between 0.1 Msun and 120 Msun
    print(np.round(test[0],3))

    ### Q2 solutions
    
    frac = quad(lambda m: imf(m),1,120) # Integrate the imf between 1 Msun and 120 Msun
    print( f"The fraction of stars that are more massive than the Sun is {np.round(frac[0],3)}")
    ...
    frac2 = quad(lambda m: imf_Mass(m),1,120) # Integrate the imf mass function between 1 and 120
    print(f"The fraction of mass in all stars that are more massive than the Sun is {np.round(frac2[0],3)}")

    # 5000 Msun cluster - how much mass is between 1 Msun and 120 Msun?
    print(f"{5000*np.round(frac2[0],4)} Msun is in stars larger than the sun for this cluster")
