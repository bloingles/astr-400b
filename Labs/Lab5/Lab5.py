...
class AbundanceMatching:
    ...
    def StellarMass(self):
        '''
        Method to compute the stellar mass using eq. 2 of Moster+13 (stellar/halo mass ratio)

        Output:
            starMass: stellar mass in Msun
        '''
...
def main() -> None:
    ...
    # Plot z = 0
    ax.plot(np.log10(mh), np.log10(MosterZ0.StellarMass()),
            linewidth = 5, label='z=0')

    # Plot z = 0
    MosterZ0_5 = AbundanceMatching(mh,0.5)
    ax.plot(np.log10(mh),np.log10(MosterZ0_5.StellarMass()),
             linewidth = 5,label='z=0.5',linestyle='-')
    # Plot z = 0
    MosterZ1 = AbundanceMatching(mh,1)
    ax.plot(np.log10(mh),np.log10(MosterZ1.StellarMass()),
             linewidth = 5,label='z=1',linestyle='-.')
    # Plot z = 0
    MosterZ2 = AbundanceMatching(mh,2)
    ax.plot(np.log10(mh),np.log10(MosterZ2.StellarMass()),
    ...
    print(f"LMC is found to have a mass of {LMC1star/1e9} giga-solar masses, which is less than the expected value of 3 giga-solar masses")
    print(f"Specifically, the value about is {np.round(LMC1star/3e9*100,2)}% of 3")
    ...
    print(f"Here, the halo mass is {np.round(LMC2star/1e9,3)}(1e9 Msun)")
