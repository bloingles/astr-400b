...
class M33AnalyticOrbit:
    ...
    def __init__(self, filename):
        ...
        self.Mdisk = ComponentMass("M31_000.txt", 2).value*1e12 # disk mass in Msun
        
        # M31 bulge
        self.rbulge = 1.0 # bulge scale length in kpc
        self.Mbulge = ComponentMass("M31_000.txt", 3).value*1e12 # bulge mass in Msun
        
        # M31 Halo
        self.rhalo = 61.58 # Halo Scale Length: use the Hernquist scale length, computed in HW5
        self.Mhalo = ComponentMass("M31_000.txt", 1).value*1e12  # halo mass in Msun
     
        
        ### ADD M33 HALO MASS HERE #### 
        self.M33halo = ComponentMass("M33_000.txt",1).value*1e12
    
    
        ### ADD M31 CIRCULAR SPEED HERE ### 
        self.Vc:int = 180 # circular speed of M31 Halo in km/s (hmwk 5)
        
        ### Fudge factor ##
        self.fudge:float = 0.6
    ...
    def DynamicalFriction(self,
                          Msat:float,
                          Vc:float,
                          r:NDArray[np.float64],
                          v:NDArray[np.float64]
                          ) -> NDArray[np.float64]:
        '''Method that returns the deceleration from dynamical
        friction felt by M33 owing to its motion through the dark
        matter halo of M31

        Arguments:
            Msat (Msun): Mass of the satellite in Msun
            Vc (km/s): Max circular velocity of the host dark matter halo
            r (kpc): The position vector of the satellite relative to the host
            v (km/s): The velocity vector of the satellite relative to the host

        Returns:
            DF: Deceleration vector owing to dynamical friction
        '''

        # magnitude of the satellite position vector
        r_mag:float = np.linalg.norm(np.vstack((r[0],r[1],r[2])),axis=0)

        # magnitude of the satellite position vector
        v_mag:float = np.linalg.norm(np.vstack((v[0],v[1],v[2])),axis=0)

        # For the Coulomb logarithm
        # bmax: current separation between M33 and M31
        bmax:float = r_mag

        # bmin: radius of a strong encounter
        # G Msat / Vc**2
        bmin:float = self.G*Msat/Vc**2

        # Coulomb logarithm
        Coulomb:float = np.log(bmax/bmin)

        # Dynamical Friction
        DF:NDArray[np.float64] = -0.428*self.G*Msat*Coulomb/r_mag**2*v/v_mag*self.fudge
        return DF
        
        
     ##### ADD v to M31Accel input #### 
    def M31Accel(self,r:NDArray[np.float64],v:NDArray[np.float64]):
        '''
        ...
        r : `np.ndarray`
            Position vector to find the acceleration (kpc)
        r : `np.ndarray`
            Velocity vector to find the acceleration (km/s)
        ...
        aDF:NDArray[np.float64] = self.DynamicalFriction(self.M33halo,self.Vc,r,v)
...
def main() -> None:
    ...
    M33Orbit_Hmwk7 = np.loadtxt("M33AnalyticOrbit.txt", dtype=orbit_type)
    ...
    ax.plot(M33Orbit['t'],M31_M33_R,'g',lw=5,ls='--',label='M31-M33 with DF')
